...
public class SupplierCoffeeServlet extends HttpServlet {
  ...
  void add() {
    EntityManagerFactory emf =
        Persistence.createEntityManagerFactory("JPAOneMany1013");
        ...
        c1.setPrice(new BigDecimal(200.0)); //BigDecimal-->可以容納很大的小數位數
        c1.setSales(10);
        c1.setTotal(5);
        c1.setSupplier(sp);
  ...
  void query(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    EntityManagerFactory emf = Persistence.createEntityManagerFactory("JPAOneMany1013");
    EntityManager em = emf.createEntityManager();
    em.getTransaction().begin();
    ...
    for (Coffee s : list) {
        ...
        //要在Coffee、Supplier加上toString的方法，否則印出來是記憶體位址
